...
function readHighScores(req, res, next) {
  db.many('SELECT (score, name, email) FROM PlayerGame, Player WHERE PlayerGame.PlayerID = Player.ID AND PlayerGame.score > 0 ORDER BY PlayerGame.score DESC LIMIT 3')
    .then(data => {
      returnDataOr404(res, data);
    })
    .catch(err => {
